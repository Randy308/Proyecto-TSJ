...
const MultiBtnDropdown = ({
  ...
  const { arrowDownIcon, arrowUpIcon, trashIcon } = useIcons();
  ...
  const selectAll = () => {
    if (contenido.length === 0) {
      toast.warning("No hay datos para seleccionar.");
      return;
    }
    setListaX((prev) => {
      const existingItem = (prev ?? []).find((item) => item.name === name);
  ...
  return (
    <>
      ...
      >
        ...
        {isLoading ? (
          <div
            className={`gap-1 absolute w-full z-30 bg-white top-10 left-0 my-4 py-8 shadow-xl  ${
              activo ? "" : "hidden"
            }`}
          >
            <Loading />
