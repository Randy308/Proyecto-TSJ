...
import type { Faceta, ResueleveFondo } from "../types";
import { AuthService } from "../services";
import { toast } from "react-toastify";

interface EditarFondoProps {
  item: ResueleveFondo;
  setShowModal: (val: boolean) => void;
}
export const EditarFondo = ({ item, setShowModal }: EditarFondoProps) => {
  const lista: Faceta[] = [
    { id: 1, nombre: "Tipo 1" },
    { id: 2, nombre: "Tipo 2" },
    { id: 3, nombre: "Tipo 3" },
  ...
  const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (titulo && tipo && selected) {
      const response = await AuthService.editResuelveFondo(
        {
          id: item.id,
          nombre: titulo,
          sala_id: selected,
      ...
      if (response.status === 200) {
        ...
        console.error("Error al editar el fondo", response);
      }
    }
  };
  ...
  useEffect(() => {
    if (item) {
      setTitulo(item.nombre || undefined);
      setSelected(item.sala_id);
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 flex flex-col gap-2">
          ...
          <span className="p-2 border">
            {" "}
            {data.sala.find((item) => item.id === selected)?.nombre}{" "}
          </span>
        </div>
        <div className="flex-1 flex flex-col gap-2">
          <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
          ...
          <span className="p-2 border">
            {" "}
            {lista.find((item) => item.id === tipo)?.nombre}{" "}
          </span>
        </div>
      </div>
