...
const Settings = ({ reversed = false }) => {
  ...
  const handleShowList = () => {
    if (settingsRef.current && optionsListRef.current) {
      const rect = settingsRef.current.getBoundingClientRect();
      const optionsListHeight = settingsRef.current.offsetHeight;
      const optionsListWidth = optionsListRef.current.offsetWidth;
      //console.log(`optionsListHeight ${optionsListHeight} optionsListWidth ${optionsListWidth} `);
      optionsListRef.current.style.top = `${
        rect.bottom - rect.height + optionsListHeight
      }px`;
      optionsListRef.current.style.left = `${rect.left - optionsListWidth - 180}px`;
    }
  };
